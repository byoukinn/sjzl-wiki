...
该脚本通过查询DTS运行含失败写入数据的运行记录，来帮助判断哪些流程写入异常，需要整改。

##  使用方法

连接至DTS数据库mongodb，要求用户空间为`Restcloud_DTS`，将以下内容复制直接执行即可观察结果。

## 各项参数

### processVersionFlag 

该项不能调整，会通过字段结构判断是否新版本，让脚本决定使用不同的逻辑查询出正确数据，请忽视该字段。
