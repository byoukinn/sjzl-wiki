...
旧版需要注意，字段不会自动寻找原表字段是哪个，需要确保原表跟目标表的字段名称一一对应，否则会导致对比匹配效果不完整。

旧版本和新版对比还少了计算并标记字段的策略，除了这两个区别之外，没有明显区别，其余请参考新版节点说明。
...
4. 生成DML：根据主键和差异结果和生成DML，用伪代码来解释：`[I/D/U操作] 增量节点全部所列字段 where 限制范围=主键 `

| 计算策略(删除数据策略) | 目标比源头流多出主键对应不上的行 | 目标与源头流有相同主键行 | 目标少于源头流的行 |
| -------- | -------- | -------- | -------- |
|计算并删除| 删除该行数据（整行删除）         | 只要对比字段中任意有一列不一致，则更新该行                   | 插入该行 |
|不计算|忽略该行|只要对比字段中任意有一列不一致，则更新该行| 插入该行 |
|计算并标记|在标记字段记录D标记| 只要对比字段中任意有一列不一致，则更新该行。并在标记字段记录U标记 | 插入该行并在标记字段记录I标记 |



然后将计算结果作为数据流，流入到下一节点。
...
### 8.15 有时候数据量相近但是数据会有错乱，比如同一个条出现两个甚至好几个，但是能写入到数据库
